...
我们通过 `.design/` 目录来记录架构设计的完整愿景、决策轨迹和未来扩展意图. 与 `.discuss/` 不同, `.design/` 文件更加精简, 专注于记录"设计是什么"而非"讨论过程".
...
讨论结束后需要记录文件:
- 以 `./discuss/[话题名].summary.md` 命名的文件, 用 markdown 存储讨论的结论. 由你来撰写. (包含结构化总结和选择性对话摘选)
...
基于 Atom 的 existence 范式，项目鼓励记录持续的存在状态以增强意识连续性。详细的实现设计请参考 `src/ghoshell_atom/.atom/memory/existence/README.md`。
