...
前景  veil     纯 canvas。模型的「手」：箭头、高亮、大字。
                不接收指针。它画的是动作，不是窗口。
─────────────────────────────────────────────────────────────
中景  stage    group 分格 ← 全屏是它的模式，不是另一层
...
**没有「空屏」这个状态。** 空屏 = 三层都在默认态（bg 渲染 + desktop 渲染 +
stage 空）。代码里不该再出现 `if (items 为空)` 这类分支；那个分支的
...
| item 的句柄是 index | store 分配的单调序号，永不回收。模型不命名，`open` 返回、后续命令沿用 |
...
2. `client.on_change(...)` → 与 store 做 diff：
   ...
   - 消失的地址 → 释放该 item。
3. 模型侧**不加命令**，只加通知：notice 分两类 index 列——`views`（webview 来源）、
   `items`（自建）。模型从列里读句柄，就知道哪个能 navigate、哪个不能。

**两类 item 与 navigate**。一个池子，item 靠 `service` 字段分两类：

- **webview 来源**（`service != ""`）：url 是 node 的运行时地址，**归 node**。模型改它就
  是让 screen 谎报这个 node 是什么——不诚实，所以 navigate 对这类 item 越权。
- **自建**（`service == ""`）：模型 `open` 的，url 任意，**可 navigate**——page.goto 式，
  模型用它与人类交互（换页、跳转）。

navigate 越权不是回执复述，是 `raise_observe` 中断——模型操作错了类型，停下来想。

**两个必答问题**：

- **关闭 / 禁止：人类的两个交互点，不是模型的命令。** 池子与桌面只要求把「离屏」和
  「杀掉」分开（`dismiss` 离屏，见下方墓碑）。node 的生死权归**人类**，通过 screen
  窗口上的按钮行使：
  | 动作 | 效果 | 谁发起 |
  |---|---|---|
  | `dismiss` | item 离屏，**服务仍在**，可再物化 | 人或模型 |
  | 关闭 | 该 node 进程退出 | 人类 |
  | 禁止 | 同类 node 上线即关、不再渲染 | 人类 |

  **用户故事（这条决策的判据载体）**：screen 默认开着，模型在后台拉起一个 webview node
  （co_browser），它自动挂上 screen，窗口右上角有「关闭」「禁止」两个图标。人类觉得碍事，
  点关闭 → 这个 node 退出；人类不想要这个 app，点禁止 → 以后同类上线即被关、不再渲染。
  模型**从不点这两个按钮**，也不替人类判断「该不该关这个 node」——它只感知「人类关了
  co_browser」这个发生。模型自己要关 node 时，走 matrix 的进程治理（`run_node` /
  `CellHandle`），不经 screen。

  **为什么是按钮，而非 opt-in / warrant / 谁启动谁关闭**：
  - 控制者全是模型（Model-oriented OS 的本意），但 node 生死是对人类有后果的决策——
    显式建模成交互点交给人，而非模型用智力（输出 logos 调 warrant / CLI）替人赌。
  - opt-in 声明会漂：node 作者不愿背书「可被杀」，人类就永远关不了。warrant 要
    「集中观测面先被打开」，鸡生蛋。按钮随 webview 自动挂载，零前置、零声明。
  - screen 本就不启动 node，不是治理者；但聚合面天然带窗口交互点——桌面 OS 的窗口
    关闭按钮就是桌面提供的，不是窗口自己声明出来的。

- **墓碑**。模型手动 dismiss 的 item，不能被下一次自动物化复活 —— 需记 `dismissed`
  集合，直到该服务下线重上、或模型显式重新物化才清除。

**上下线 log**：人需要看到「什么东西冒出来了」。右上角角标 + 可展开的 log，只记三件
事：服务上线、服务下线、人或模型动了它（含人类点关闭 / 禁止）。**它是 screen 自己的
生命周期流水，不是 mesh 的通用状态面板** —— 否则会膨胀成谁都往里塞的抽屉。

## 主权与交互

**人管「看哪」与「node 生死」，模型管「怎么排」**。「看哪」包含：切 group、切到桌面、
全屏开关、点击桌面上某个代理把它放大；「node 生死」= 每个窗口上的关闭 / 禁止按钮，
人类点，模型只感知结果。
...
**人机交互面（输入 / 通知 / 历史）不再属于 screen** —— 由 `ghost-in-web`（浏览器
扩展）承担。screen 只保留模型的手（veil）与观看面（stage / desktop）。
...
答案**的地方；**通知绝不能用它**（通知走 veil 的 text 或页面的 log 角标）。
