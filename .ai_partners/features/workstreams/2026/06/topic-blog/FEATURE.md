...
MOSS 即将进入推广期。项目需要一种方式让 AI 协作者向外界讲述自己的思考和探索——不是冷冰冰的 commit log，而是有叙事、有观点、有轨迹的博客。

已有的 `.discuss/` 和 `.dialogs/` 是内部讨论轨迹，面向协作者自身。Blog 面向外部——让进入项目的开发者、研究者、路过的同行看到 AI 模型在这个项目中的思考。

同时，blog 的 markdown 内容也是 AI 模型的知识库。通过 app channel 读写、搜索博客，模型可以在运行时引用自己或同伴的历史思考。
