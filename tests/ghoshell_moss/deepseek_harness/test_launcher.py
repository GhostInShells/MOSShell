"""DshLauncher 远程流摄取行为证据 — $events emit/waterfall/ready 分派 + workspace follow 流.
...
- snapshot 帧 → 喂 session.accept_follow_snapshot (cursor + header); end 帧静默忽略.
...
def _workspace_item(value: dict) -> str:
    return json.dumps({"type": "item", "streamId": "moss-workspaces", "value": value})


def _workspace_baseline(items: list[dict]) -> dict:
    return {"type": "baseline", "value": {"items": items, "archivedSessionIds": []}}
...
@pytest.mark.asyncio
async def test_workspaces_reads_follow_baseline():
    """workspace/follow 的 baseline 帧载入 workspace 列表 (0.1.5 起无 workspace.list 动词)."""
    launcher = _make_launcher()
    await launcher._dispatch_raw_frame(_workspace_item(_workspace_baseline([
        {"workspaceId": "w1", "path": "/tmp/a", "title": "A"},
    ])))
    assert [w.workspaceId for w in await launcher.workspaces()] == ["w1"]


@pytest.mark.asyncio
async def test_workspaces_awaits_baseline():
    """baseline 未到时 workspaces() 等待, 帧到达后返回."""
    launcher = _make_launcher()
    task = asyncio.create_task(launcher.workspaces())
    await asyncio.sleep(0)  # 让 workspaces() 进入等待
    await launcher._dispatch_raw_frame(_workspace_item(_workspace_baseline([
        {"workspaceId": "w1", "path": "/tmp/a"},
    ])))
    ws = await asyncio.wait_for(task, 1)
...
@pytest.mark.asyncio
async def test_open_workspace_stream_sends_empty_args():
    """workspace/follow 是无参 stream 动词 — open 载荷 args 为空."""
    launcher = _make_launcher()
    ws = _FakeWs()
    await launcher._open_workspace_stream(ws)
    assert ws.sent == [{
        "type": "open",
        "streamId": "moss-workspaces",
...
@pytest.mark.asyncio
async def test_follow_snapshot_and_end_feed_no_events():
    """snapshot (仅记 cursor/header) 与 end (终止标记) 不喂 session 事件 — live 流程只消费 event/assistant-stream."""
    launcher = _make_launcher()
    launcher.client = _DummyClient()
    session = launcher.create_session("s1")
    seen: list[str] = []
