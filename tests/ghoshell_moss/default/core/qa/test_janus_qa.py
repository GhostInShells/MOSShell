...
async def test_round_trip_approval():
    ...
    mgr = JanusQAManager(issuer='ghost-1')

    asker = mgr.asker('safemode')
    watcher = mgr.watch('safemode')

    # watcher receives incoming questions
    seen: list[JanusQA] = []
    ...
    # let distributor copy question to watcher
    ...
    # responder replies
    answer = r_qa.question.approve('ok')
    r_qa.reply(answer)
...
async def test_reply_reject():
    ...
    answer = seen[0].question.reject('busy')
    seen[0].reply(answer)
...
async def test_cancel():
    ...
    # cancel broadcast reaches watcher
    await asyncio.sleep(0.05)
    assert r_qa.done()
...
async def test_first_reply_wins():
    """With multiple watchers on the same namespace, the first reply is the
    accepted answer; the second is silently dropped."""
    mgr = JanusQAManager(issuer='ghost-1')
    asker = mgr.asker('ns')
    w1 = mgr.watch('ns')
    w2 = mgr.watch('ns')
    ...
    # both watchers see the question
    assert len(seen1) == 1
    assert len(seen2) == 1

    # first watcher replies; second also tries
    seen1[0].reply(seen1[0].question.approve('first'))
    # second watcher's reply should raise (it already sees its copy as
    # replied? no — they are separate copies with separate _replied, so
    # both can push; the owner accepts the first)
...
async def test_auto_cancel_on_context_exit():
    """QA async context manager auto-cancels on exception / exit."""
    mgr = JanusQAManager(issuer='ghost-1')
    asker = mgr.asker('ns')

    async with asker.ask_approval('will cancel') as qa:
        pass  # exit without resolving
...
async def test_qameta_is_set_on_answer():
    ...
    assert final.meta.issuer == 'ghost-1'  # watcher uses mgr issuer
