...
观察者持有 moments 容器, observe() 生产 Moment, add_echoes / inject_percepts /
...
def test_observe_advances_echoes_container_and_resets_need_observe():
    obs = BaseMomentsObserver(max_size=5)
    obs.add_echoes([Message.new().with_content("done")], need_observe=True)
    assert obs.need_observe() is True
    m = obs.observe()
    assert obs.need_observe() is False
    # observe 产出的 moment.previous 衔接上一轮结果.
    assert "done" in _texts(m.previous_echoes_messages())
...
def test_on_echoes_add_fires_with_appended():
    obs = BaseMomentsObserver(max_size=5)
    calls = []
    obs.on_echoes_add(lambda msgs, need: calls.append((_texts(msgs), need)))
    obs.add_echoes([Message.new().with_content("hi")], need_observe=True)
    assert calls == [(["hi"], True)]


def test_on_echoes_add_not_fired_for_empty_echo():
    """空 append 不 fire 回调, 但 need_observe 信号仍被置位 (观察仍需发生)."""
    obs = BaseMomentsObserver(max_size=5)
    calls = []
    obs.on_echoes_add(lambda msgs, need: calls.append(msgs))
...
def test_on_moment_fires_with_produced_moment():
    obs = BaseMomentsObserver(max_size=5)
    seen = []
    obs.on_moment_created(lambda m: seen.append(m))
    m = obs.observe()
    assert seen == [m]
...
def test_clear_resets_state():
    ...
    obs.add_echoes([Message.new().with_content("x")], need_observe=True)
    obs.observe()
    # 让 need_observe 重新为 True, 断言 clear 彻底复位.
    obs.add_echoes([Message.new().with_content("y")], need_observe=True)
    assert obs.need_observe() is True
    obs.clear()
    assert obs.need_observe() is False
    assert obs.moments() == []
...
def test_three_coroutines_run_different_cadences():
    ...
    - consumer (0.005s): add_echoes 喂入上一轮结果
    - observer (0.02s): 注册 on_moment, 被动记录每个产出的 Moment

    协程只在 await 处协作切换; observe / add_echoes 内部无 await, 因此对
    ...
    async def consumer(n=10):
        for i in range(n):
            ...
            obs.add_echoes([Message.new().with_content(f"res-{i}")], need_observe=True)

    async def observer(n=6):
        # 观测角色: 注册同步 on_moment 钩子, 在对方生产 Moment 的瞬间被动记录.
        obs.on_moment_created(lambda m: observed.append(m))
        for _ in range(n):
    ...
    res_texts = _texts([msg for m in observed for msg in m.previous_echoes_messages()])
...
# ============================================================
# echo-drain / need_observe 信号契约 (暴露 _peek_moment drain 缺陷)
# ============================================================

def test_add_echoes_empty_with_observe_sets_flag():
    """add_echoes([], need_observe=True) 应置位 need_observe — 空信号也需触发观察.

    当前 Echoes.add_echoes 只在非空 append 时置位, 空信号丢失.
    """
    obs = BaseMomentsObserver(max_size=5)
    obs.add_echoes([], need_observe=True)
    assert obs.need_observe() is True
...
def test_echo_drain_observe_signal_reaches_previous():
    """drain 返回 (空消息, observe=True): 产出的 moment.previous.need_observe 应为 True.

    当前 _peek_moment 经 add_echoes 空 append 丢失该 observe 信号.
    """
    obs = BaseMomentsObserver(max_size=5)
    obs.with_echoes_drain("k", lambda: ([], True))
    m = obs.observe()
    assert m.previous is not None
    assert m.previous.need_observe is True


def test_echo_drain_not_duplicated_across_peek_then_observe():
    """peek 预览后 observe, drain 结果不应被重复累积.

    drain 数据直接落入 Echoes; 若 observer 另留一份缓冲, observe 会把
    ...
    obs.with_echoes_drain("k", drain)
    obs.peek()
    m = obs.observe()
    prev_texts = _texts(m.previous_echoes_messages())
