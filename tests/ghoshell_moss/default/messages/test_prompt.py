...
The @ protocol: a line starting with ``@`` is a file reference (path to end
of line). Resolved files become Messages; unresolvable ones stay inline.
``expose_file_meta`` (external flag) wraps files with the meta layer
(tag="file" + path/type/size) — discardable, rendered by as_contents.
"""
...
def test_prompt_line_start_at_only(tmp_path: Path):
    _tree(tmp_path)
    # inline @ is NOT a reference — only line-start
    blocks = message_from_prompt("see @notes.md here", base_dir=tmp_path)
    assert len(blocks) == 1
    assert blocks[0].contents[0]["text"] == "see @notes.md here"
