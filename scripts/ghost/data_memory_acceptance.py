"""Deterministic DataMemory persistence smoke test (no LLM/network required)."""
...
from ghoshell_moss.ghosts.data import DataMemory
...
def _history_text(memory: DataMemory) -> str:
...
def verify(root: Path) -> None:
    ...
    first = DataMemory(root, "acceptance", auto_commit_every=2)
    ...
    reopened = DataMemory(root, "acceptance", auto_commit_every=2)
    ...
    print("PASS: DataMemory write -> commit -> close -> reopen -> render")
    print(f"root={root}")
...
def main() -> None:
    ...
    if args.root:
        ...
        return
    with tempfile.TemporaryDirectory(prefix="moss-data-memory-") as directory:
