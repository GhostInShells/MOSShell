"""File editor engine (vendor patch — see UPSTREAM.md).

MOSS carries this file verbatim from openhands-aci except for the removals
documented in ``UPSTREAM.md``:

- ``binaryornot`` (external) → inline ``_is_binary`` (stdlib magic-byte check)
- ``openhands_aci.linter.DefaultLinter`` → removed; ``enable_linting`` is a
  silent no-op (arg kept for signature compatibility)
- ``openhands_aci.utils.shell.run_shell_cmd`` → removed; directory view uses
  ``pathlib`` traversal (also removes a shell-injection surface)
- ``.md_converter.MarkdownConverter`` → removed; viewing an Office / PDF /
  audio file raises ``FileValidationError`` instead of converting to markdown

Behaviour otherwise unchanged. The MOSS-facing contract layer wraps this
engine and translates its exceptions — see ``ghoshell_moss.contracts.file_editor``.
"""
...
from .prompts import TEXT_FILE_CONTENT_TRUNCATED_NOTICE
from .results import CLIResult, maybe_truncate


def _is_binary(path: Path, sample_size: int = 8192) -> bool:
    """Detect binary files by null-byte presence in the first sample.

    Replacement for ``binaryornot.check.is_binary``. Null bytes are a strong
    signal for binary content; almost all text encodings (utf-8, latin-1,
    utf-16 with BOM handling elsewhere) avoid them in valid text. False
    positives are acceptable for an editor: it errs toward "refuse to edit"
    rather than corrupting a binary payload.
    """
    try:
...
class OHEditor:
    ...
    def __init__(
        ...
        Args:
            ...
            workspace_root: Root directory used for relative path suggestions in error messages.
                Must be an absolute path. If None, no path suggestions will be provided.
        """
    ...
    def view(self, path: Path, view_range: list[int] | None = None) -> CLIResult:
        ...
        Directory listing is not supported — use bash/glob for exploration.
        Directories raise EditorToolParameterInvalidError at validate_path.
        """
    ...
    def validate_path(self, command: Command, path: Path) -> None:
        ...
        if path.is_dir():
            raise EditorToolParameterInvalidError(
                'path',
                path,
                f'The path {path} is a directory. This tool operates on files only; use bash/glob for directory listing.',
            )
