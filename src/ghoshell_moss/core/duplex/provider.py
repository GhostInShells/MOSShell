...
from ghoshell_moss.core.concepts.channel import Channel, ChannelProvider, Brokers
...
class DuplexChannelProvider(ChannelProvider):
    ...
    def __init__(
        ...
        self._brokers: Brokers | None = None
    ...
    async def _bootstrap_channels(self) -> None:
        """递归启动所有的 broker."""
        brokers = Brokers.new(self.channel, self.container)
    ...
    async def _clear_running_status(self) -> None:
        ...
        clearing = []
        async for path, broker in self._brokers.iter():
            if broker.is_available():
                clearing.append(broker.clear())
    ...
    async def _handle_default_event(self, event: ChannelEvent) -> None:
        ...
        try:
            if model := CommandCallEvent.from_channel_event(event):
                ...
                _ = self.loop.create_task(self._handle_idle_event(model))
            elif model := PauseEvent.from_channel_event(event):
                _ = self.loop.create_task(self._handle_pause(model))
            elif model := ClearEvent.from_channel_event(event):
                _ = self.loop.create_task(self._handel_clear(model))
            else:
    ...
    async def _handel_clear(self, event: ClearEvent):
        ...
        try:
            broker = self._brokers.get(channel_name)
            if not broker:
                return
            if not broker.is_available():
                return
            # 执行 clear 命令.
            task = asyncio.create_task(broker.clear())
            await task
        except asyncio.CancelledError:
            pass
        except Exception as e:
    ...
    async def _handle_sync_channel_meta(self, event: SyncChannelMetasEvent) -> None:
        ...
        async for path, broker in self._brokers.iter():
            if broker.is_available():
                refresh_tasks.append(broker.refresh_meta())
        await asyncio.gather(*refresh_tasks)

        for channel_path, broker in self._brokers.broker_map.items():
            metas[channel_path] = broker.meta()
