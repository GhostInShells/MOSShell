...
console = Console()
# todo: 考虑把 console 的实例化统一位置. 现在每个文件都有实例化.
...
def list_contracts(
    ...
    all_contracts = host.manifest.contracts()
...
def _display_command_detail(cmd):
    ...
    # 重点展示接口定义
    if hasattr(cmd, '__prompt__'):
        console.print(Panel(cmd.__prompt__(), title="Interface Prompt", border_style="yellow"))
