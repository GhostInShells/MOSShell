...
class NetworkNamespace:
    ...
    def __init__(self, env: Environment):
        ...
        self.moss_ns = "MOSS"
        # matrix 网络通讯根.
        self.matrix_ns = "MOSS/matrix"
        # 所有 hosts 声明 (跨 scope), 用于 all_hosts().
        self.hosts_ns = f"{self.matrix_ns}/hosts"

        scope = env.network_scope
        # 当前 scope 的通讯子空间.
        self.scope_ns = f"{self.matrix_ns}/scopes/{scope}"

        # scope 内 host 唯一性标记.
        self.host_alive_key = f"{self.scope_ns}/host-alive"

        # scope 下的各类通讯子空间.
        scope_ns = self.scope_ns
        self.channels_ns = '/'.join([scope_ns, 'channels'])
        self.cells_ns = '/'.join([scope_ns, 'cells'])
        self.topic_ns = '/'.join([scope_ns, 'topics'])
        self.signal_ns = '/'.join([scope_ns, 'signals'])
        self.stream_ns = '/'.join([scope_ns, 'streams'])
        self.output_ns = '/'.join([scope_ns, 'outputs'])
