...
class LayoutSnapshot:
    ...
    def __init__(self, name: str, state_class: type[rx.State], storage_dir: Path) -> None:
        ...
        self._data_full: dict[str, object] = {}
        self._init_defaults()
    ...
    async def refresh(self, root_state: rx.State) -> None:
        ...
        for name, type_hint in self._state_class.__annotations__.items():
            ...
            self._data_full[name] = self._serialize_full(val, type_hint)
            self._data[name] = self._summarize(val, type_hint)

    def get(self) -> dict[str, object]:
        return self._data

    def get_full(self) -> dict[str, object]:
        """返回全量数据（不截断），供 save_chapter 等持久化场景使用。"""
        return dict(self._data_full)

    def save(self) -> None:
        self._storage_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {"version": SNAPSHOT_VERSION, "data": self._data_full}
    ...
    def _init_defaults(self) -> None:
        for name, type_hint in self._state_class.__annotations__.items():
            ...
            self._data_full[name] = _default_for(type_hint)

    @staticmethod
    def _serialize_full(val: object, type_hint: type) -> object:
        """将运行时值序列化为全量数据（不截断），供持久化使用。

        - str            → 原样（不截断）
        - list[str]      → 全量列表
        - list[Image]    → 仅记数量（locator 由 Ghost 在 save_chapter 时传入）
        - Image.Image    → 哨兵值 1（用于校验数量）
        - None           → 按类型给默认哨兵值
        """
        if val is None:
